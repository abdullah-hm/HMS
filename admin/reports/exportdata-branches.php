...
if($query->num_rows > 0){ 
    $delimiter = ","; 
    $filename = "branch-data_" . date('Y-m-d') . ".csv"; 
     
    // Create a file pointer 
    $f = fopen('php://memory', 'w'); 
     
    // Set column headers 
    $fields = array('Branch ID', 'Branch Code', 'Branch Name', 'Branch Address'); 
    fputcsv($f, $fields, $delimiter); 
     
    // Output each row of the data, format line as csv and write to file pointer 
    while($row = $query->fetch_assoc()){ 
        $lineData = array($row['id'], $row['BranchCode'], $row['BranchName'], $row['BranchAddress']); 
