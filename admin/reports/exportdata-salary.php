...
if($query->num_rows > 0){ 
    ...
    // Set column headers 
    $fields = array('Salary ID', 'mployee ID', 'Employee Name', 'Basic Salary', 'Over Time', 'Total Salary', 'Salary Issued Date'); 
    fputcsv($f, $fields, $delimiter); 
     
    // Output each row of the data, format line as csv and write to file pointer 
    while($row = $query->fetch_assoc()){ 
        $lineData = array($row['PayrollID'], $row['EmpID'], $row['EmpFLname'], $row['Basic'], $row['OverTime'], $row['TotalSalary'], $row['SalaryIssuedDate']); 
